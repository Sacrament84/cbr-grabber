                ...
                dir ('cbr-backend/kustomize') {
                    container(name: 'kubectl') {
                        sh """
                               ...
                               kubectl rollout status deployment/staging-cbr-backend -n staging || (echo "failed to deploy"; kubectl rollout undo deploy/staging-cbr-backend -n staging; exit 1)
                           """
                ...
                dir ('cbr-frontend/kustomize') {
                    container(name: 'kubectl') {
                        sh """
                               ...
                               kubectl apply -k overlays/staging
                               kubectl rollout status deployment/staging-cbr-frontend -n staging || (echo "failed to deploy"; kubectl rollout undo deploy/staging-cbr-frontend -n staging; exit 1)
                           """
                ...
                dir ('cbr-backend/kustomize') {
                    container(name: 'kubectl') {
                        sh """
                               ...
                               kubectl rollout status deployment/production-cbr-backend -n production || (echo "failed to deploy"; kubectl rollout undo deploy/production-cbr-backend -n production; exit 1)
                           """
                ...
                dir ('cbr-frontend/kustomize') {
                    container(name: 'kubectl') {
                        sh """
                               ...
                               kubectl apply -k overlays/production
                               kubectl rollout status deployment/production-cbr-frontend -n production --timeout=5min || (echo "failed to deploy"; kubectl rollout undo deploy/production-cbr-frontend -n production; exit 1)
                           """
